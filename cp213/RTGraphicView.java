...
public class RTGraphicView extends JComponent {
    ...
    /**
     * Inner class for allowing the triangle to be dragged in order to change its
     * size. The cursor is changed depending on whether the triangle can be resized.
     */
	...
	public void mouseDragged(final MouseEvent evt) {
	    ...
	    if (this.canDrag) {
		RTGraphicView.this.model.setBase(evt.getX() / RTGraphicView.this.scaleX);
		RTGraphicView.this.model
			.setHeight((RTGraphicView.this.getHeight() - evt.getY()) / RTGraphicView.this.scaleY);
	    }
	}

	/**
	 * The mouse moved: check if it is over the dragable regions and adjust the
	 * cursor accordingly.
	 */
	@Override
	public void mouseMoved(final MouseEvent evt) {

	    if (RTGraphicView.this.handle.contains(RTGraphicView.this.getMousePosition())) {
		this.canDrag = true;
		RTGraphicView.this.setCursor(Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR));
	    } else {
		this.canDrag = false;
		RTGraphicView.this.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
	    }
	}
    }
    ...
    private static final Insets BORDER_INSETS = new Insets(BORDER_SIZE, BORDER_SIZE, BORDER_SIZE, BORDER_SIZE);
    private static final Border BORDER = new EmptyBorder(BORDER_INSETS);
    /**
     * Define the mouse handle information.
     */
    private static final int HANDLE_SIZE = 6;
    private static final Dimension HANDLE_DIMENSION = new Dimension(RTGraphicView.HANDLE_SIZE,
	    RTGraphicView.HANDLE_SIZE);
    ...
    /**
     * Overrides JComponent's paintComponent method. Draws the triangle and adds a
     * handle to the top corner of the triangle.
     *
	...
	this.right.setLocation(this.model.getBase() * this.scaleX + BORDER_SIZE, this.left.getY());
	this.top.setLocation(this.right.getX(), this.getHeight() - BORDER_SIZE - this.scaleY * this.model.getHeight());
    ...
    /**
     * Calculate the triangle to window scaling. The same scaling is applied to both
     * the base and height in order to preserve the 'look' of the triangle.
     */
