...
public class RTSameView extends JPanel {
	...
	public void actionPerformed(final ActionEvent evt) {
	    ...
	    double base = RTSameView.this.model.getBase();
	    double height = RTSameView.this.model.getHeight();

	    if (this.direction == 1) {
		// increase both by 1
		RTSameView.this.model.setBase(base + 1);
		RTSameView.this.model.setHeight(height + 1);
	    } else if (this.direction == -1) {
		// decrease both by 1
		RTSameView.this.model.setBase(base - 1);
		RTSameView.this.model.setHeight(height - 1);
	    } else {
		// direction == 0 → set both to the average
		double avg = (base + height) / 2.0;
	...
	public void propertyChange(final PropertyChangeEvent evt) {
	    ...
	    // Base/Height matches only when they are equal
	    if (base == height) {
		RTSameView.this.baseHeight.setText(DECIMAL_FORMAT.format(base));
	    } else {
		RTSameView.this.baseHeight.setText("");
	    }

	    // Always update hypotenuse
	    RTSameView.this.hypo.setText(DECIMAL_FORMAT.format(RTSameView.this.model.getHypotenuse()));
	}
    }
