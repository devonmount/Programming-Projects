 ...
 * node link can be updated, but not the node countedItem, in order to avoid
 * moving countedItem between nodes. Data structures must be reordered by moving
 * nodes.
 *
...
public class TreeNode<T extends Comparable<T>> {
    ...
    // Attributes
    private CountedItem<T> countedItem = null; // the node countedItem
    ...
    /**
     * Creates a new TreeNode with countedItem countedItem and null links to its
     * child TreeNodes.
     *
     * @param countedItem The countedItem to store in the node.
     */
    public TreeNode(final CountedItem<T> countedItem) {
	this.countedItem = countedItem;
    }

    /**
     * Performs an inorder traversal of a tree adding node countedItem to a list.
     *
     * @param node A TreeNode.
     * @param list The final list of node countedItem.
     */
    private final void inOrderAux(final TreeNode<T> node, final ArrayList<CountedItem<T>> list) {
	if (node != null) {
	    this.inOrderAux(node.getLeft(), list);
	    list.add(new CountedItem<T>(node.getCountedItem()));
    ...
    /**
     * Performs an preorder traversal of a tree adding node countedItem to a list.
     *
     * @param node A TreeNode.
     * @param list The final list of node countedItem.
     */
    ...
    /**
     * Returns this node countedItem. Not copy safe as it returns a reference to the
     * countedItem, not a copy of the countedItem.
     *
     * @return this node countedItem.
     */
    public CountedItem<T> getCountedItem() {
    ...
    /**
     * Returns a list of the countedItem in the current tree. The list contents are
     * in order from smallest to largest.
     *
     * Not thread safe as it assumes contents of the tree are not changed by an
     * external thread during the loop.
     *
     * @return The contents of this tree as a list of countedItem.
     */
    ...
    /**
     * Returns a list of the countedItem in the current tree. The list contents are
     * in node level order starting from the root node. Helps determine the
     * structure of the tree.
     *
     * Not thread safe as it assumes contents of the tree are not changed by an
     * external thread during the loop.
     *
     * @return this tree countedItem as a list of countedItem.
     */
	...
	if (this != null) {
		...
		node = queue.remove(0);
		// Add a copy of the node countedItem to the list of countedItem
		list.add(new CountedItem<T>(node.getCountedItem()));
    ...
    /**
     * @return a string version of this node including the countedItem and height.
     */
    @Override
    public String toString() {
	return "D: " + this.countedItem + "; H: " + this.height;
    }
